...
if (isset($_GET['id'])) {
    $blocked_id = (int)$_GET['id'];

    // Delete the blocked date entry
    $query = "DELETE FROM blocked_dates WHERE id = ? AND lecturer_id = ?";
    $stmt = $con->prepare($query);
    $stmt->bind_param("ii", $blocked_id, $lecturer_id);
    $stmt->execute();

    if ($stmt->affected_rows > 0) {
        $_SESSION['success_message'] = "Blocked date deleted successfully.";
    } else {
        $_SESSION['error_message'] = "No blocked date was deleted. It may not exist or you lack permission to delete it.";
    }
} else {
    $_SESSION['error_message'] = "Invalid blocked date ID.";
}
